...
describe('NestDB Snapshot Store', () => {
  ...
  it('stores Snapshots', () => {
    ...
    return snapshots.store('Test', 'foo', 'v1', snapshot)

      .then(() => new Promise((y, n) => db.find({}, (err, docs) => err ? n(err) : y(docs))))

      .then(docs => docs.should.eql([{
        _id: JSON.stringify({dom: 'Test', key: 'foo', ver: 'v1'}),
  ...
  it('fetches Snapshots', () => {
    return Promise.resolve()
      ...
      .then(() => new Promise(y => db.insert([
        {
          ...
          _id: JSON.stringify({dom: 'Test', key: 'bar', ver: 'v2'})
        }, {
          _id: JSON.stringify({dom: 'Test', key: 'foo', ver: 'v2'}),
          las: new Date('2011-12-13'),
          had: {foo: 42},
          sta: {foo: 'bar'}
        }
      ...
      .then(() => snapshots.fetch('Test', 'foo', 'v2'))
  ...
  it('updates existing Snapshots', () => {
    ...
    return snapshots.store('Test', 'foo', 'v1', snapshot1)

      .then(() => snapshots.store('Test', 'bar', 'v1', snapshot1))

      .then(() => snapshots.store('Test', 'foo', 'v1', snapshot2))

      .then(() => new Promise((y, n) => db.find({}, (err, docs) => err ? n(err) : y(docs))))

      .then(docs => docs.should.eql([{
        _id: JSON.stringify({dom: 'Test', key: 'bar', ver: 'v1'}),
        las: new Date('2011-12-13'),
        had: {foo: 21},
        sta: {foo: 'bar'}
      }, {
